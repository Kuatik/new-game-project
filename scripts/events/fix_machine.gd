...
const PROVOD_1 = preload("uid://ncdravd1dl6m")
...
func create_color_circle(color_name: String) -> TextureRect:
	var rect = TextureRect.new()
	rect.texture = PROVOD_1
	#rect.color = COLORS[color_name]
	rect.modulate = COLORS[color_name]
	#rect.clip_contents = true
	rect.custom_minimum_size = Vector2(385, 64)
	rect.size = Vector2(385, 64)
...
func _on_left_click(event: InputEvent, rect: TextureRect, color_name: String):
...
func _on_right_click(event: InputEvent, rect: TextureRect, color_name: String):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
...
func remove_pair(left_rect: TextureRect, right_rect: TextureRect):
